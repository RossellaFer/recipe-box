const Recipe = ({title, id, ingredients, steps, setShowEditModal, handleDelete, showModal, setShowModal}) => {
    ...
    return (
        <>
            <div id={id}>
                <header className='basis-1/8 pb-4 pt-2 flex justify-between'>
                ...
                <footer className='basis-1/8 py-4'>
                    <button type="button" onClick={() => setShowModal(true)} className='px-5 py-2 mr-2 text-sm leading-5 rounded-md font-semibold bg-teal-300 border-teal-300 border-2 hover:bg-transparent shadow-xl'>Add new recipe</button>
                </footer>
            </div>
        </>
    )
